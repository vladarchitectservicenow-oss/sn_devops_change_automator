# sn_devops_change_automator

## Architecture
```mermaid
graph TD
    SN[ServiceNow] -->|REST| sn_devops_change_automator
    sn_devops_change_automator -->|Store| DB[Tables]
    sn_devops_change_automator -->|Generate| Report[Reports]
```
## Quick Start
```bash
git clone https://github.com/vladarchitectservicenow-oss/sn_devops_change_automator.git
cd sn_devops_change_automator && python3 src/cli.py --help
```
## ROI
| Approach | Hours/Year | Cost |
|----------|-----------|------|
| Manual | 40 | $3,400 |
| With sn_devops_change_automator | 5 | $425 |
...
## Overview
sn_devops_change_automator is a production-grade ServiceNow scoped application developed by Vladimir Kapustin under AGPL-3.0.

## Architecture
```mermaid
graph TD
    SN[ServiceNow Instance] -->|REST| sn_devops_change_automator
    sn_devops_change_automator -->|Store| DB[x_sn_devops_change_automator_tables]
    sn_devops_change_automator -->|Output| Report[Reports MD/JSON/CSV]
...
## ROI Analysis
| Metric | Manual Process | With sn_devops_change_automator |
|--------|---------------|-------------|
...
# Run scan
POST /api/x_sn_devops_change_automator/scan
...
See `Validation/TEST CASES/sn_devops_change_automator/test_suite_SOP.md`
...
## Support
- GitHub Issues: https://github.com/vladarchitectservicenow-oss/sn_devops_change_automator/issues
- ServiceNow Community: Tag `sn_devops_change_automator`


---

## Detailed Product Overview
sn_devops_change_automator is a production-ready scoped application engineered specifically for the ServiceNow platform, targeting enterprises that demand high automation, rigorous compliance, and deep observability. It bridges critical gaps in native ServiceNow capabilities by providing custom business logic, reusable Script Includes, and robust REST endpoints that integrate with external DevOps, security, analytics, and collaboration tools.
...
sn_devops_change_automator is packaged using the ServiceNow Application Repository and Studio standards. It ships with a complete `sys_app` definition, scoped tables, Business Rules, Script Includes, Client Scripts, UI Policies, UI Actions, scheduled jobs, and inbound REST APIs. The application follows ServiceNow naming conventions: the scope is `x_sn_devops_change_automator`, and all artifacts are prefixed consistently to avoid collisions with other applications.

In large organizations, manual execution of the workflows handled by sn_devops_change_automator typically consumes 40–120 hours per year per team. At a blended hourly rate of $85/hour, this translates to $3,400–$10,200 annually in labor cost, not including opportunity cost, human error, or compliance penalties. sn_devops_change_automator reduces this workload to fewer than 5 hours per year through automation, delivering an 87%+ cost reduction with immediate payback.
...
4. Enter the repository URL: `https://github.com/vladarchitectservicenow-oss/sn_devops_change_automator.git`.
...
|----------|------|---------|-------------|
| `x_sn_devops_change_automator.logging.level` | Choice | INFO | DEBUG, INFO, WARN, ERROR |
| `x_sn_devops_change_automator.scan.chunk_size` | Integer | 500 | Records per batch for large tables |
| `x_sn_devops_change_automator.scan.delta.enabled` | Boolean | true | Enable incremental scanning |
| `x_sn_devops_change_automator.output.format.default` | Choice | md | Default report format |
| `x_sn_devops_change_automator.api.timeout_seconds` | Integer | 30 | REST call timeout |
| `x_sn_devops_change_automator.encryption.key_id` | String | — | Reference to sys_encryption_context |
| `x_sn_devops_change_automator.healthcheck.enabled` | Boolean | true | Enable health endpoint |

Roles:
- `x_sn_devops_change_automator.admin`: Full configuration, deployment, and report access.
- `x_sn_devops_change_automator.user`: Read-only report viewing and health-check access.
- `x_sn_devops_change_automator.api`: Service account role for CI/CD pipelines.
...
| Memory exhaustion | Chunk size too large for JVM/Python heap | Reduce `x_sn_devops_change_automator.scan.chunk_size` | Tune heap per instance sizing |
...
## Support & Community
- **GitHub Issues:** https://github.com/vladarchitectservicenow-oss/sn_devops_change_automator/issues
- **Discussions:** https://github.com/vladarchitectservicenow-oss/sn_devops_change_automator/discussions
- **ServiceNow Community:** Tag `sn_devops_change_automator` in posts.
